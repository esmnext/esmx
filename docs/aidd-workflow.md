...
graph TD
    ...
    I -->|❌ 不达标| J["🔄 直接返工<br/>回到测试编写"]
    
    J --> D
    
    I -->|✅ 达标| K["👨‍💻 人类审查<br/>读取 .ai/standards/review.md"]
    
    K --> K1["📝 更新工单进度<br/>人类审查完成"]
    
    K1 --> L{"🤔 审查通过?"}
    
    L -->|❌ 需修改| M["📋 反馈收集<br/>回到需求分析"]
    
    L -->|🔄 其它改进| N["📋 创建新工单"]
    
    M --> M1["📝 更新工单进度<br/>反馈收集完成"]
    
    M1 --> B
    N --> A
    
    L -->|✅ 通过| O["📊 AI 输出完整报告<br/>读取 .ai/standards/report.md"]
    
    O --> O1["📝 更新工单进度<br/>完整报告生成完成"]
    ...
    P -->|✅ 完整| R["🚀 AI 提交PR<br/>读取 .ai/standards/pull-request.md"]
    ...
    class A,A1,B,B1,C,C1,D,D1,E,E1,F,F1,G,G1,H,H1,K1,M,M1,O,O1,Q,R,R1,T,V,X,X1,Y,Z aiProcess
    class K,N humanProcess
    class I,L,P,U decision
    class S,W output
    class J feedback
```
