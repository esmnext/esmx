...
function renderHtml(layout: Layout): string {
    return (
        ...
        `<path d="M16 2C7.16 2 2 12 2 17c0 0 3-3 5-3s5 3 9 3 9-3 13 3c0-5-5.16-16-13-16z" fill="#fff"/>` +
        `</svg>` +
        `</div>` +
        `<h1 style="font-size: 2rem; font-weight: 800; color: var(--esmx-text-primary); margin-bottom: 12px;">Lit Micro-App</h1>` +
        `<p style="font-size: 1.125rem; color: var(--esmx-text-secondary); margin-bottom: 32px;">` +
...
export class LitApp extends BaseApp {
    ...
    renderToString(): Promise<string> {
        return Promise.resolve(renderHtml(this.layout));
    }
}
