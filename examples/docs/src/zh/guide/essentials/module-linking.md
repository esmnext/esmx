...
## 模块链接 vs Module Federation

两者都能在独立构建的应用间共享代码，但机制不同：

| | Esmx 模块链接 | Module Federation |
|---|---|---|
| 运行时 | 原生 ESM + Import Maps（浏览器原生） | Webpack/Rspack 运行时容器 |
| 开销 | 零额外运行时 | Federation 运行时 + 共享作用域协商 |
| SSR | 一等公民，SEO 友好 | 需额外配置，历史上较脆弱 |
| 锁定 | 标准 `import`，不绑定打包器 | 依赖特定打包器插件与配置 |
| 版本 | 显式 provider，单一所有者 | 运行时共享作用域协商 |

由于链接基于浏览器自身的模块系统，没有沙箱、没有私有加载器、运行时无需额外产物——同一份 `import` 在浏览器、SSR 与测试中都一致工作。

## 相关

- [路径别名](/zh/guide/essentials/alias) —— 链接模块间的跨服务导入如何解析
- [样式](/zh/guide/essentials/styles) —— 链接的远程模块如何拥有并共享自己的 CSS
