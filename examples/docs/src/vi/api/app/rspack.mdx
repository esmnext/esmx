...
description: Công cụ xây dựng Rspack của Esmx Framework, cung cấp khả năng xây dựng ứng dụng hiệu suất cao, hỗ trợ phát triển và xây dựng ứng dụng tiêu chuẩn và ứng dụng HTML, tích hợp nhiều bộ xử lý tài nguyên và cấu hình tối ưu hóa.
head:
  - - meta
    - property: keywords
      content: Esmx, Rspack, công cụ xây dựng, xây dựng ứng dụng, ứng dụng HTML, TypeScript, CSS, xử lý tài nguyên, tối ưu hóa hiệu suất
---
...
Sử dụng trình quản lý gói để cài đặt `@esmx/rspack` làm phụ thuộc phát triển:
...
- `css`: Cách xuất CSS, có thể chọn 'css' (tệp độc lập) hoặc 'js' (đóng gói vào JS), mặc định được chọn tự động dựa trên môi trường: môi trường sản xuất sử dụng 'css' để tối ưu hóa bộ nhớ đệm và tải song song, môi trường phát triển sử dụng 'js' để hỗ trợ cập nhật nóng (HMR)
