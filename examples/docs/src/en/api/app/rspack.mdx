---
titleSuffix: "Esmx Rspack Build Tool"
description: "High-performance build tool for Esmx applications, supporting standard and HTML apps with built-in processors and optimizations."
head:
  - - "meta"
    - name: "keywords"
      content: "Esmx, Rspack, build tool, app build, HTML app, TypeScript, CSS, loaders, performance"
---
...
The Rspack package provides APIs to create and configure Rspack apps, supporting both standard and HTML apps.

## Install

Install `@esmx/rspack` as a dev dependency:
...
- `css`: output mode ('css' for separate files, 'js' inline). Defaults to 'css' in production for caching and parallel loading; 'js' in development for HMR
