...
Several APIs on this page read or write files at framework-conventional paths. They are not configurable through `EsmxOptions` — if you rename or move them the framework will not find them.

| Path | What |
|------|------|
| `src/entry.node.ts` | Your `EsmxOptions` default export. Read by the CLI on `esmx dev` / `esmx build` / `esmx start`. |
| `src/entry.server.ts` | SSR entry. Exports `default async (rc: RenderContext) => { rc.html = ... }`. Loaded by `esmx.render()`. |
| `src/entry.client.ts` | Browser hydration entry. Bundled and referenced from the SSR HTML via `rc.moduleEntry()`. |
| `dist/client/manifest.json` | Generated by `esmx build`. The client bundle's federation manifest. |
| `dist/server/manifest.json` | Generated by `esmx build`. The server bundle's federation manifest. |
| `dist/node/src/entry.node.mjs` | Generated by `esmx build`. The compiled `entry.node.ts` that `esmx start` imports. |
| `dist/index.mjs` | Bootstrap shim `esmx build` writes; not user-edited. |

Code samples on this page assume those paths.
