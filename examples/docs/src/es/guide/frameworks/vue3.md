---
titleSuffix: Ejemplo de aplicación Vue3 SSR con el framework Esmx
description: Aprende a crear una aplicación Vue3 SSR desde cero utilizando el framework Esmx. Este tutorial muestra el uso básico del framework, incluyendo la inicialización del proyecto, configuración de Vue3 y configuración de archivos de entrada.
head:
  - - meta
    - property: keywords
      content: Esmx, Vue3, Aplicación SSR, Configuración TypeScript, Inicialización de proyecto, Renderizado en el servidor, Interacción en el cliente, API de composición
---

# Vue3

Este tutorial te guiará en la creación de una aplicación Vue3 SSR (Server-Side Rendering) utilizando el framework Esmx. A través de un ejemplo completo, mostraremos cómo usar Esmx para crear una aplicación con renderizado en el servidor.
...
<template>
    <div>
        <h1><a href="https://www.esmnext.com/guide/frameworks/vue3.html" target="_blank">Inicio rápido con Esmx</a></h1>
...
export default {
     ...
     * @description Crea una instancia del servidor HTTP, integra el middleware de Esmx y maneja las solicitudes SSR
     * @param esmx Instancia del framework Esmx, proporciona middleware y funciones de renderizado
     */
    async server(esmx) {
        const server = http.createServer((req, res) => {
            // Usar el middleware de Esmx para manejar la solicitud
...
2. `server`: Responsable de crear y configurar el servidor HTTP, integrando el middleware de Esmx para manejar solicitudes SSR.
...
¡Ahora has creado con éxito una aplicación Vue3 SSR utilizando el framework Esmx! Visita http://localhost:3000 para ver el resultado.